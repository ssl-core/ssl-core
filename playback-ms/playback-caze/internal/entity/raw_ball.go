...
func NewRawBallFromSSLDetectionBall(detection_ball_pb *detection.SSL_DetectionBall) *RawBall {
	confidence := util.SetDefaultIfNil(detection_ball_pb.GetConfidence(), 0)
	position := []float32{util.SetDefaultIfNil(detection_ball_pb.GetX(), 0), util.SetDefaultIfNil(detection_ball_pb.GetY(), 0), util.SetDefaultIfNil(detection_ball_pb.GetZ(), 0)}
