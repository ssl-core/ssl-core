...
class ManualFirstModule : public AModule {
 public:
  ManualFirstModule(int id, float sleep_duration_ms)
    : AModule(id, sleep_duration_ms) { assert(id == 0); }
...
class FirstModule : public AModule {
 ...
 public:
  FirstModule(int id, float sleep_duration_ms)
    : AModule(id, sleep_duration_ms) { assert(id == 0); }
...
int main(int argc, char* argv[]) {
  ...
  auto last = reinterpret_cast<LastModule*>(modules.back().get());
